# Lens — Image Relevance Workspace: deployment
...
Run npm ci, npm run migrate and npm start. Set HOST=0.0.0.0, PORT, DATABASE_URL, DATA_DIR on persistent storage and OLLAMA_URL. Pull the documented models and keep the worker enabled. Expose only the application over HTTPS, not PostgreSQL or Ollama. Actual vision accuracy is not certified by deterministic tests.
...
Existing checks: `npm test; npm run test:browser`. After deployment, check desktop/mobile layout and the developer link, then exercise the real application workflow. Verify health, authentication, writes and logout where applicable. Configure the backend before expecting application data or jobs to work.
